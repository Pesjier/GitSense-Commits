...
## 10:19:45 PM
This log details development activity primarily focused on integrating with the SMHI API and displaying weather data on an ESP32-based device with a graphical user interface (LVGL). The changes occurred between 9:20 PM and 10:18 PM on December 4, 2025.

### File-Specific Updates:

**`c:\C++Saker\projects\PA1484\PA1484\project\SMHIAPI\SMHIAPI.cpp`**

This file, responsible for SMHI API interactions, saw extensive modifications related to handling HTTPS requests and JSON deserialization:

*   **Early Debugging (9:20 PM - 9:22 PM):** The initial state of the `httpsGetJson` function involved fetching the entire HTTP response as a `String` (`http.getString()`), converting it to a `charArray` using `malloc` and `toCharArray`, and then deserializing the JSON. Debugging comments suggest initial issues with direct string deserialization.
*   **Memory/Payload Debugging (9:22 PM - 9:26 PM):** Several `Serial.printf` statements were uncommented to log `Content-Length`, `payload.length()`, `ESP.getFreeHeap()`, and the raw payload, indicating efforts to diagnose issues related to response size and memory consumption.
*   **Stream-Based Deserialization Attempts (9:26 PM - 9:35 PM):**
    *   An incomplete attempt to pipe the HTTP response to a generic `Stream` was made.
    *   This evolved into using `StreamString` (`#include <StreamString.h>`) and `http.writeToStream(stream)` to capture the response.
    *   The `deserializeJson` call was repeatedly modified to try different ways of consuming the `StreamString` content: first, directly from the `StreamString*` pointer, then its dereferenced object (`*stream`), and finally, its C-style string representation (`stream->c_str()`). A `Serial.print(stream->c_str());` was added for debugging the stream's content.
*   **Reverting and Further Debugging (9:41 PM - 10:01 PM):**
    *   The stream-based capturing and debugging of `StreamString` were removed or commented out.
    *   Debugging output shifted to `client->availableForWrite()` and later `client->available()` with `client->read()` in a manual loop, suggesting direct inspection of the underlying `WiFiClientSecure` stream.
    *   At one point, `deserializeJson(doc, "");` was used, indicating a placeholder or a test for deserialization failure with empty input.
    *   **HTTP/1.0 Force (9:47 PM):** `http.useHTTP10(true);` was introduced before `http.begin()`, possibly to resolve connection or protocol negotiation issues.
    *   There was a brief, erroneous attempt to deserialize from `http.get`, followed by a return to `deserializeJson(doc, http.getString());`, indicating a step back to the simpler (but potentially less memory-efficient) method.
*   **Direct Client Stream Deserialization (10:04 PM - 10:18 PM):**
    *   The manual client read loop was removed.
    *   Attempts were made to read specific parts of the `WiFiClientSecure` stream using `client->readStringUntil('{')`, `client->readString()`, and `client->readStringUntil('\r')` or `'\n'` before passing the remaining client stream (or a portion of it) to `deserializeJson` via `deserializeJson(doc, *client);`. This indicates ongoing efforts to correctly extract and parse the JSON payload while bypassing HTTP headers.

**`c:\C++Saker\projects\PA1484\PA1484\project\project.ino`**

This is the main Arduino sketch, integrating UI and API calls.

*   **Initial Setup and UI (10:08:27 PM):**
    *   Includes numerous libraries for UI (`TFT_eSPI`, `LilyGo_AMOLED`, `LV_Helper`, `lvgl`), WiFi (`WiFiHandler`), and custom components (`Dropdown`, `BootScreen`, `Graph`, `WeatherForecastElement`, `Linegraf`, `SettingsScreen`, `SMHIStationsAndParameters`, `SMHIAPI`).
    *   Defines global UI objects for a tileview (`tileview`, `t0`, `t1`, `t2`), main screen labels (`t0_label`, `t0_text1`, `t0_text2`), and components for forecast and graph displays.
    *   The `create_ui()` function sets up a multi-tile user interface:
        *   `t0`: Displays a "Weather app v1.0 Grupp 9" welcome screen.
        *   `t1`: Configured for a 7-day weather forecast, populated with example data ("Karlskrona", dates, various weather types).
        *   `t2`: Configured for a line graph (`Linegraf`) with example coordinates, and displays a `graphTitle` and `graphDescription`.
    *   Event listeners are set up for the `SettingsScreen`:
        *   `AddListenerToLocation`: Triggers an SMHI forecast API call for a selected location (using longitude/latitude), parses the JSON response using `SMHIForecastParser`, and updates the forecast elements with real-time data.
        *   `AddListenerToCondition`: Updates the graph's title and description based on a selected parameter.
    *   The `setup()` function initializes hardware (AMOLED), sets up SMHI data, displays a boot screen, connects to WiFi using `WiFiHandler` (with "jesper's Galaxy A52" SSID and "pqss3103" password), and performs a test API call using `buildURL` and `serializeJson`.
    *   `settings->LoadDefaultValues()` is called at the end of setup.
*   **API Response Debugging (10:08:53 PM):** A minor change was introduced in the `AddListenerToLocation` lambda to `serializeJson(doc, str); Serial.println(str);`, which prints the full JSON document received from the SMHI forecast API to the serial monitor. This was likely added for debugging the format or content of the API response.

### Patterns and Recurring Elements:

1.  **SMHI API as a Central Data Source:** Both files heavily rely on fetching data from the SMHI (Swedish Meteorological and Hydrological Institute) API, either for station/parameter metadata or actual forecast data. URLs indicate calls to `opendata-download-metobs.smhi.se` and `opendata-download-metfcst.smhi.se`.
2.  **JSON Deserialization Challenges:** There's a persistent struggle in `SMHIAPI.cpp` to efficiently and correctly deserialize JSON data, with repeated attempts to use different methods (string conversion, `StreamString`, direct client stream) and extensive serial debugging. This suggests that large JSON payloads or memory constraints on the ESP32 platform are causing difficulties.
3.  **Extensive Serial Debugging:** Throughout the development process, `Serial.print` and `Serial.println` statements are frequently added, modified, and commented/uncommented to monitor URLs, HTTP codes, memory, and raw data, indicating an iterative, debug-driven development approach.
4.  **LVGL-based UI:** The `project.ino` file consistently uses the LVGL library for creating a structured graphical user interface with tiles, forecast elements, and graphs.
5.  **Reactive UI Design:** The UI responds to user input via dropdowns in a `SettingsScreen`, which then triggers API calls and updates other UI elements, demonstrating a reactive programming pattern.
6.  **Explicit Memory Management Concerns:** Repeated checks of `ESP.getFreeHeap()` and attempts at stream-based parsing highlight a focus on memory optimization, crucial for embedded systems like the ESP32.
7.  **Future Timestamps:** All changes are dated December 2025, which is unusual for a development log and might indicate an incorrect system clock setting or a planned development timeline.

## 11:20:09 PM
The provided log details a series of iterative changes across two C++ files, `SMHIAPI.cpp` and `project.ino`, and one parser file `SMHIForecastParser.cpp`, primarily focusing on robust HTTPS JSON data retrieval and parsing for an ESP32-based weather application.

### File-Specific Updates:

#### `c:\C++Saker\projects\PA1484\PA1484\project\SMHIAPI\SMHIAPI.cpp`

This file underwent extensive modifications, predominantly within the `httpsGetJson` function, over a period from 9:20 PM to 11:16 PM on 12/4/2025.

**Key Changes to `httpsGetJson`:**

*   **Initial Implementation (9:20 PM - 9:22 PM):** The function initially retrieved the entire HTTP response as a `String` (`http.getString()`), then converted it to a `char*` using `malloc` and `toCharArray` for JSON deserialization (`deserializeJson(doc, charArray)`). Debugging comments for `contentLength`, `payload`, and `ESP.getFreeHeap()` were present but commented out.
*   **Debugging and Stream Attempts (9:22 PM - 9:35 PM):**
    *   Briefly re-enabled `contentLength` and `payload` logging.
    *   Introduced and then removed attempts to use `Stream` and `StreamString` for reading, reflecting an effort to handle potentially large responses more efficiently and avoid memory issues associated with `http.getString()`.
    *   The `deserializeJson` call was modified to try using the `StreamString` object (`deserializeJson(doc, stream)`) and then its C-string representation (`deserializeJson(doc, stream->c_str())`).
*   **Direct Client Interaction & HTTP/1.0 (9:41 PM - 9:58 PM):**
    *   Experimented with printing `client->availableForWrite()`, then directly reading and printing raw bytes from `client->available()` in a loop.
    *   Briefly added `http.useHTTP10(true)` to the `HTTPClient` instance, suggesting attempts to resolve connection or compatibility issues. This was later removed and re-added.
    *   At one point, `deserializeJson(doc, http.getString())` was used, reverting to the full string approach.
    *   Changed `deserializeJson(doc, "");` and `deserializeJson(doc, http.get);` (syntax error) in debugging, indicating an isolated problem with the deserialization part.
*   **Header Skipping and Stream Refinements (10:01 PM - 10:22 PM):**
    *   Introduced `client->readStringUntil('{')` to skip HTTP headers before JSON parsing, then `client->readStringUntil('\r')`, and `client->readStringUntil('\n')`.
    *   Moved towards directly passing the `WiFiClientSecure` object (dereferenced as `*client`) to `deserializeJson`, intending to utilize its streaming capabilities directly.
    *   Added conditional header skipping `if(contentLength == -1) { client->readStringUntil('\n'); }` to handle unknown content lengths.
*   **Manual Stream Processing with `stringstream` (10:33 PM - 11:16 PM):**
    *   Extensive changes involved manually reading characters from `client` into a `std::stringstream` (requiring `#include <sstream>`).
    *   Introduced complex logic to differentiate and filter out HTTP headers (specifically chunked encoding size lines and `\r\n` sequences) from the actual JSON payload by tracking `doneReading` and `foundQuoteMark` flags.
    *   Despite accumulating the JSON data into a `stringstream`, the `deserializeJson` function continued to incorrectly attempt deserialization from the now-empty raw `*client` stream, rather than from the populated `stringstream`. This appears to be a persistent logical error throughout the latter half of the changes.

#### `c:\C++Saker\projects\PA1484\PA1484\project\project.ino`

This is the main Arduino sketch, which integrates the SMHI API functionality and handles the UI (LVGL) and WiFi.

*   **Initial Setup (10:08 PM):** The file includes various libraries for display, UI, WiFi, and SMHI-related parsing. It initializes WiFi with hardcoded credentials and sets up an LVGL-based UI with multiple "tiles" for weather information, a graph, and a home screen.
*   **SMHI Integration in UI (10:08 PM):** Event listeners are added to a `SettingsScreen` to react to user changes for location and weather conditions.
    *   **Location Listener:** When a new location is selected, it constructs an SMHI forecast URL using `stringstream`, fetches JSON data via `httpsGetJson`, parses it with `SMHIForecastParser`, and updates the `WeatherForecastElement` objects in the UI. Initial implementation included serializing and printing the `JsonDocument` for debugging.
    *   **Condition Listener:** Updates graph titles and descriptions based on selected weather parameters.
*   **Error Handling Refinement (10:25 PM):** An explicit check `if(!forecast.getDataFromJSON(doc))` was added in the `Location` listener to detect and log parsing failures, and the success message was refined to "Fetched data!".

#### `c:\C++Saker\projects\PA1484\PA1484\project\ParserStuff\SMHIForecastParser.cpp`

This file defines the `SMHIForecastParser` class responsible for interpreting the JSON data received from the SMHI forecast API.

*   **Core Functionality (10:24 PM):** The `extractForecastData` and `getDataFromJSON` methods are central, parsing a `JsonDocument` (either from a string or an existing `doc` object). They iterate through a `timeSeries` array, filter data points to only include "noon" times, and extract various forecast parameters (temperature, wind speed, humidity, pressure, precipitation, weather symbol). It includes logic to handle cases where data fields might be strings or numbers.
*   **Minor Correction (10:25 PM):** Changed `return 1;` to `return true;` in `getDataFromJSON` for better boolean consistency.

### Timestamps of Significant Changes:

*   **12/4/2025, 9:20:12 PM:** Initial robust JSON retrieval using `http.getString()` and `char*` conversion.
*   **12/4/2025, 9:30:59 PM:** Introduction of `<StreamString.h>` and `http.writeToStream(stream);` marks a shift towards stream-based processing.
*   **12/4/2025, 9:32:13 PM:** `deserializeJson` first attempts to use `stream` directly.
*   **12/4/2025, 9:42:13 PM:** Removal of stream-based components and temporary `deserializeJson(doc, "");`.
*   **12/4/2025, 9:47:41 PM:** `http.useHTTP10(true)` introduced.
*   **12/4/2025, 9:47:55 PM:** `deserializeJson(doc, http.getString())` temporarily restores string-based deserialization.
*   **12/4/2025, 9:58:07 PM:** Direct raw client data reading `while (client.available())` introduced for debugging.
*   **12/4/2025, 10:04:19 PM:** `deserializeJson(doc, *client)` adopted for direct stream deserialization.
*   **12/4/2025, 10:08:27 PM:** `project.ino` shows the application's core logic with UI and initial SMHI API call setup.
*   **12/4/2025, 10:08:53 PM:** Debugging `serializeJson(doc, str); Serial.println(str);` added in `project.ino`.
*   **12/4/2025, 10:21:26 PM:** Conditional header skipping based on `http.getSize()` in `SMHIAPI.cpp`.
*   **12/4/2025, 10:24:54 PM:** `SMHIForecastParser.cpp` content first appears, demonstrating detailed JSON parsing logic.
*   **12/4/2025, 10:59:09 PM:** `<sstream>` included and `std::stringstream` used for manual client reading in `SMHIAPI.cpp`, indicating an attempt to construct the JSON payload manually.

### Patterns and Recurring Elements:

*   **Iterative Debugging of JSON Retrieval:** The `httpsGetJson` function in `SMHIAPI.cpp` shows a continuous struggle and experimentation with different methods to retrieve JSON data over HTTPS, primarily due to concerns about memory limitations on embedded systems (ESP32) when using `http.getString()`, and challenges in parsing chunked HTTP responses or skipping headers when directly streaming. Many changes involve adding and removing `Serial.print` statements to inspect network client state and received data.
*   **Memory Management Concerns:** The shift from `http.getString()` (which buffers the entire response in memory) to `StreamString` and then direct `*client` stream processing, along with `malloc`/`toCharArray` for `String` to `char*` conversion, indicates a recurring concern for efficient memory usage when handling potentially large JSON payloads on the ESP32.
*   **Header/Payload Separation:** Repeated attempts to skip HTTP headers (e.g., `client->readStringUntil('{')`, checking `contentLength == -1`, and the complex `stringstream` logic with `foundQuoteMark`) highlight the difficulty in reliably extracting the JSON payload from the raw HTTP response stream.
*   **SMHI API Integration:** Both `SMHIAPI.cpp` and `project.ino` consistently interact with the SMHI public API endpoints (`opendata-download-metobs.smhi.se`, `opendata-download-metfcst.smhi.se`) for weather data.
*   **UI Updates:** The `project.ino` file consistently uses LVGL to create and update graphical user interface elements based on the fetched weather data, indicating a real-time or near real-time display requirement for the application.
*   **Hardcoded Credentials:** The `WiFiHandler` in `project.ino` uses hardcoded WiFi SSID and password, which is a common but insecure practice in development for embedded systems.
*   **Arduino Platform Focus:** The use of `Serial.println`, `ESP.getFreeHeap()`, `WiFiClientSecure`, `HTTPClient`, `LilyGo_AMOLED`, and various LVGL functions confirms this project is developed for an Arduino-compatible ESP32 board with a display.